import { u as useTranslation, aO as useGuestFormSettings, bb as useUpdateGuestFormSettings, r as reactExports, j as jsxRuntimeExports, aJ as LoaderCircle, bc as ClipboardList, B as Button, a7 as Plus, t as ue, bd as Variant_text_dropdown, w as Label, K as ChevronUp, M as ChevronDown, I as Input } from "./index-BNayfcmF.js";
...
function backendToLocal(b) {
  return {
    ...
    customQuestions: b.customQuestions.map((q) => ({
      ...
      type: q.questionType === Variant_text_dropdown.dropdown ? "dropdown" : "text",
      options: q.options,
      required: q.required
    }))
  };
  ...
  return {
    ...
    customQuestions: local.customQuestions.map(
      (q) => ({
        id: q.id,
        labelText: q.labelText,
        questionType: q.type === "dropdown" ? Variant_text_dropdown.dropdown : Variant_text_dropdown.text,
        options: q.options,
        required: q.required
      })
    )
...
function GuestFormSettingsPage() {
  ...
  const isDirty = saved === null || JSON.stringify(config) !== JSON.stringify(saved);
